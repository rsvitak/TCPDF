...
class tcpdf_cmssignature {
  ...
  public function __construct() {
     if (defined('TCPDF_CMSSIGNATURE_LOG')) $this->logFile=TCPDF_CMSSIGNATURE_LOG;
     else $this->writeLog=false;
  }
  ...
  public function __destruct() {
    if($this->writeLog) {
      ...
      foreach($arrLog as $line) {
        ...
        $newhead = $head; //str_pad($head, 10, " ");
        $ct = rtrim(substr($line, strpos($line, ":")+1));
        $newline = '['.$newhead.': '.$ct.'] ';
        $newlines .= $newline;
      }
      $logs .= $newlines;
      file_put_contents($this->logFile, $logs.PHP_EOL, FILE_APPEND);
    }
  }
